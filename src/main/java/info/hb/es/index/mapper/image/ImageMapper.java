...
public class ImageMapper implements Mapper {
	...
	public static class Builder extends Mapper.Builder<Builder, ImageMapper> {
		...
		public ImageMapper build(BuilderContext context) {
			...
			for (FeatureEnum featureEnum : features.keySet()) {
				...
				// add feature mapper
				featureMappers.put(featureName, binaryField(featureName).store(true).includeInAll(false).index(false)
						.build(context));
				...
				if (featureMap.containsKey(HASH)) {
					...
					for (String h : hashes) {
						...
						hashMappers.put(hashFieldName, stringField(hashFieldName).store(true).includeInAll(false)
								.index(true).build(context));
					}
				}
			}
			...
			return new ImageMapper(name, threadPool, context.indexSettings(), features, featureMappers, hashMappers,
					metadataMappers);
		}
	}
		...
		public Mapper.Builder parse(String name, Map<String, Object> node, ParserContext parserContext)
			...
			for (Map.Entry<String, Object> entry : metadatas.entrySet()) {
				...
				builder.addMetadata(metadataName,
						parserContext.typeParser(fieldType).parse(metadataName, metadataMap, parserContext));
			}
	...
	public ImageMapper(String name, ThreadPool threadPool, Settings settings,
			Map<FeatureEnum, Map<String, Object>> features, Map<String, Mapper> featureMappers,
	...
	public void parse(ParseContext context) throws IOException {
		...
		for (ObjectObjectCursor<FeatureEnum, Map<String, Object>> cursor : features) {
			...
			try {
				...
				context.doc().add(
						new BinaryDocValuesField(name() + "." + featureEnum.name(), new BytesRef(parsedContent)));
		...
		if (!metadataMappers.isEmpty()) {
			try {
				Metadata metadata = ImageMetadataReader.readMetadata(new BufferedInputStream(new BytesStreamInput(
						content, false)), false);
