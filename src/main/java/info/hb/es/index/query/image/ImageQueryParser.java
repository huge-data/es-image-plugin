...
import info.hb.es.index.mapper.image.ImageMapper;
	...
	public Query parse(QueryParseContext parseContext) throws IOException, QueryParsingException {
			...
			while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
				if (token == XContentParser.Token.FIELD_NAME) {
					...
					if ("feature".equals(currentFieldName)) {
						...
						throw new QueryParsingException(parseContext.index(), "[image] query does not support ["
								+ currentFieldName + "]");
					}
				}
			}
		...
		if (image != null) {
			...
			GetResponse getResponse = client.get(
					new GetRequest(lookupIndex, lookupType, lookupId).preference("_local").routing(lookupRouting)
							.fields(lookupFieldName).realtime(false)).actionGet();
			if (getResponse.isExists()) {
				...
				if (getField != null) {
					...
					try {
						feature = featureEnum.getFeatureClass().newInstance();
						feature.setByteArrayRepresentation(bytesReference.array(), bytesReference.arrayOffset(),
		...
		if (hashEnum == null) { // no hash, need to scan all documents
			...
			if (limit > 0) { // has max result limit, use ImageHashLimitQuery
				...
				for (int h : hash) {
					query.add(new BooleanClause(new ImageHashQuery(new Term(hashFieldName, Integer.toString(h)),
							luceneFieldName, feature, imageScoreCache, boost), BooleanClause.Occur.SHOULD));
				}
				return query;
			}
		}
